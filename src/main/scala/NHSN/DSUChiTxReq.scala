...
import NHDSU.CHI._
import NHDSU._
import NHDSU.CHI.CHIOp.REQ
import freechips.rocketchip.scie.SCIE.opcode
...
// --------------------- Modules declaration --------------------- //
  val queue                  = Module(new Queue(new CHIBundleREQ(chiBundleParams), entries = dsuparam.nrSnTxLcrdMax, pipe = true, flow = false, hasFlush = false))
...
// ------------------- Reg/Wire declaration ---------------------- //
  ...
  val inFlit                 = WireInit(0.U.asTypeOf(new CHIBundleREQ(chiBundleParams)))
